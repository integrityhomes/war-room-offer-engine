...
def _install_deferred_stability() -> bool:
    # one_load_sources_safe is imported while One-Load UI modules are still being
    # created. Installing the full stability workflow here would recurse into the
    # same partially initialized module. The bootstrap installs a title hook only;
    # the full wrapper composition runs after startup imports are complete.
    try:
        import app_stability_bootstrap as bootstrap
    except ImportError:
        try:
            from . import app_stability_bootstrap as bootstrap
        except ImportError:
            from war_room_offer_engine import app_stability_bootstrap as bootstrap
    return bool(bootstrap.install_deferred_hook())
